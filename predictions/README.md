...
## Paper A/B experiments (`pm_btc_paper`, `kalshi_mm_paper`)

An old strategy against its rewrite, run side by side in paper on the same markets.
Generated by `polymarket-btc-options/ab_report.py` and `prediction-mm/scripts/ab_report.py`,
pushed daily by those repos' launchd jobs.

- Required: `generated_at`, `experiment`, `paper` (must be `true`), `started`, `unit`
  (`window` or `day`), `status` (`collecting` | `verdict`), `min_units_for_verdict`,
  `headline`, `arms`, `comparisons`, `kill_criteria`, `caveats`.
- Each arm: `name`, `role` (exactly one `control`), `description`, `units`, `fills`,
  `size`, `pnl`; integers where counts, numbers where money.
- Each comparison's `verdict` is `collecting` | `better` | `worse` | `indistinguishable`,
  and anything but `collecting` is refused while the sample is below
  `min_units_for_verdict` -- the size fixed before the first fill. That is the rule the
  feed exists to keep: a running total must never be published as a result.
